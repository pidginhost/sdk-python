# coding: utf-8

"""
    Pidgin Host API

    The Pidgin Host API allows you to manage Services and resources within the Pidgin Host cloud in a simple, programmatic way using conventional HTTP requests. All of the functionality that you are familiar with in the Pidgin Host control panel is also available through the API, allowing you to script the complex actions that your situation requires.

    The version of the OpenAPI document: 1.0.0 (v1)
    Contact: support@pidginhost.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from pidginhost_sdk.models.paginated_dedicated_server_list import PaginatedDedicatedServerList

class TestPaginatedDedicatedServerList(unittest.TestCase):
    """PaginatedDedicatedServerList unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PaginatedDedicatedServerList:
        """Test PaginatedDedicatedServerList
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PaginatedDedicatedServerList`
        """
        model = PaginatedDedicatedServerList()
        if include_optional:
            return PaginatedDedicatedServerList(
                count = 123,
                next = 'http://api.example.org/accounts/?page=4',
                previous = 'http://api.example.org/accounts/?page=2',
                results = [
                    pidginhost_sdk.models.dedicated_server.DedicatedServer(
                        id = 56, 
                        hostname = '', 
                        status = null, 
                        price = '-80728880', 
                        next_invoice = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(), 
                        created = '', 
                        billing_cycle = '', 
                        server_status = '', 
                        ips = '', 
                        os_name = '', )
                    ]
            )
        else:
            return PaginatedDedicatedServerList(
                count = 123,
                results = [
                    pidginhost_sdk.models.dedicated_server.DedicatedServer(
                        id = 56, 
                        hostname = '', 
                        status = null, 
                        price = '-80728880', 
                        next_invoice = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(), 
                        created = '', 
                        billing_cycle = '', 
                        server_status = '', 
                        ips = '', 
                        os_name = '', )
                    ],
        )
        """

    def testPaginatedDedicatedServerList(self):
        """Test PaginatedDedicatedServerList"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
