# coding: utf-8

"""
    Pidgin Host API

    The Pidgin Host API allows you to manage Services and resources within the Pidgin Host cloud in a simple, programmatic way using conventional HTTP requests. All of the functionality that you are familiar with in the Pidgin Host control panel is also available through the API, allowing you to script the complex actions that your situation requires.

    The version of the OpenAPI document: 1.0.0 (v1)
    Contact: support@pidginhost.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from pidginhost_sdk.models.cluster_detail import ClusterDetail

class TestClusterDetail(unittest.TestCase):
    """ClusterDetail unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ClusterDetail:
        """Test ClusterDetail
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ClusterDetail`
        """
        model = ClusterDetail()
        if include_optional:
            return ClusterDetail(
                id = 56,
                status = 'pending',
                name = '',
                generation = '',
                cluster_type = '',
                kube_version = '',
                price_per_month = '-80728880015280.7',
                price_per_hour = 1.337,
                features = [
                    'cert-manager'
                    ],
                features_ready = True,
                kubeconfig_valid_until = '',
                ipv4_address = '',
                ipv6_address = '',
                dual_stack = True,
                protected = True,
                talos_version = '',
                talos_upgrade_available = True,
                talos_next_version = '',
                storage_quota_gb = 56,
                last_pool_used_bytes = 56,
                last_storage_sync_at = ''
            )
        else:
            return ClusterDetail(
                id = 56,
                status = 'pending',
                generation = '',
                cluster_type = '',
                kube_version = '',
                price_per_month = '-80728880015280.7',
                price_per_hour = 1.337,
                features_ready = True,
                kubeconfig_valid_until = '',
                ipv4_address = '',
                ipv6_address = '',
                dual_stack = True,
                talos_version = '',
                talos_upgrade_available = True,
                talos_next_version = '',
                storage_quota_gb = 56,
                last_pool_used_bytes = 56,
                last_storage_sync_at = '',
        )
        """

    def testClusterDetail(self):
        """Test ClusterDetail"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
