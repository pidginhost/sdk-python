...
All notable changes to the PidginHost Python SDK are documented here.
...
- `NodeMetricsResponse.mem`, `.maxmem`, `.netin` and `.netout` widened to 64-bit.
...
In this SDK, class names match the component names; attributes are snake_case (`reboot_required`).
...
- Regenerated alongside the Go SDK 0.4.1 patch release (no functional changes for Python).
